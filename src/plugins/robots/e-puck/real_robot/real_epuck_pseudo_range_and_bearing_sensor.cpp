...
namespace argos
{
   ...
   void CRealEPuckPseudoRangeAndBearingSensor::UpdateValues()
   {
       ...
       if(in_receivedpacketinbuffersize > 0)
       {
           ...
           for(int index = 0; index < in_receivedpacketinbuffersize; index += 3)
           {
               SReceivedPacket* sNewPacket = new SReceivedPacket();
               sNewPacket->RobotId  = pun_databuffer[index];
               sNewPacket->Range    = (Real)pun_databuffer[index + 1];
               sNewPacket->Bearing  = CRadians(
                                              (((Real)pun_databuffer[index + 2]) / 255.0f)
                                                                                * CRadians::TWO_PI);
